...
Naming observations surfaced while writing `.defs/vocab.html`. These items have now been **decided and implemented (2026-07-24)** — this file is the decision record. Each item records the original name, the problem, the options considered, and, in a **Resolution (2026-07-24)** line, the decision that was implemented.
...
- **Blast radius (alias rename):** `definitions.py`, `registry.py`, `submission.py`, `operations.py`, `inspection.py`, README example. Type alias only — no runtime, schema, or fixture change. (Dataclass promotion additionally touches every construction/destructuring site.)
- **Resolution (2026-07-24):** Promoted to a frozen dataclass `PipelineIdentity(key, version)` with validation in `__post_init__`; public entry points require it (a tuple is rejected). The duplicated `_validate_pipeline` helpers in `operations.py` and `inspection.py` were replaced by one `validate_pipeline_identity` in `definitions.py`.
...
- **Resolution (2026-07-24):** Export exactly the record types the public API returns. `StageExecutionRecord` and `StageControlRecord` join the exported surface (they were already leaking through `StageExecutionSummary`, the retry/cancellation results, and the control operations' returns); `PipelineRunRecord` and `WorkItemRecord` stay internal. The rule is now "exported iff returned by the public API."
...
- **Resolution (2026-07-24):** Resolved by item 2's rule — with `StageExecutionRecord` and `StageControlRecord` now exported, `StageAttemptRecord` is no longer an asymmetric lone `*Record`. It keeps the `Record` name because it is the persisted row shape.
...
- **Resolution (2026-07-24):** No rename. The vocab's Stage Attempt row now documents `terminal_summary` as a diagnostic-only field, not a result payload.
...
- **Resolution (2026-07-24):** Implemented — the local in `_stage_workflow_name` was renamed to `name_slug` with a clarifying comment that it is a slug for the DBOS workflow name, not an identity or content hash.
...
- **Resolution (2026-07-24):** Standardized on the full spelling: `WorkInput.input_reference`, `AdmissionPayload.input_reference`, and `submit(execution_config_reference=)`. This also collapsed a third spelling the item originally missed — `submit`'s `config_ref` parameter, now `execution_config_reference`. DB columns are unchanged.
...
- **Resolution (2026-07-24):** Kept internal — `CampaignWorkIdentity` stays out of `__all__`. The docs continue to describe work identity as the composite key `(campaign key, work key)`.
...
- **Resolution (2026-07-24):** No rename. The vocab rows already separate the operation (Admission) from its payload (Admission Payload).

## 9. `PipelineStageMismatchError` is exported but can never be caught by applications

- **Current name:** `PipelineStageMismatchError`, exported in `dr_platform.__all__`.
- **Problem:** The error can never be caught by applications — admission catches it internally (`admission.py`) and surfaces mismatches only through the dispatcher's logs and the internal `AdmissionSummary`. Exporting it implies a catchable contract that does not exist.
- **Resolution (2026-07-24):** Un-exported from `dr_platform.__all__`; the class remains internal to admission. The vocab's Admission concept now has no Exported Names row, matching that admission raises no catchable exported error.
