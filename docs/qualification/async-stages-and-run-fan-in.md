...
`9af0ecbf357123464299f6f949dcaf6883d59342`, the declared admission and run
...
unknown submissions. The executor had a 200-worker limit and reached 182 active
workers.
...
The state-gated handoff took **1.553305 seconds**, below the **7.2-second
rate-equivalent bound** for 100,000 admissions per hour, and completed at
**115.882 successful handoffs per second**. Cleanup completed for all workflows.
Cancellation-request-to-cleanup was 14.709 ms minimum, 150.749
ms p50, 415.928 ms p95, and 517.929 ms maximum. Release-to-cleanup for the 20
cancelled coroutines was 5.710 ms minimum, 14.226 ms p50, 324.987 ms p95, and
338.151 ms maximum.
...
|---|---:|---:|---:|---:|---:|---:|
| Dedicated checkpoint queue delay | 220 | 0.018 ms | 0.036 ms | 4.404 ms | 8.994 ms | 44.678 ms |
| Stage checkpoint application-pool wait | 200 | 0.000 ms | 215.673 ms | 710.590 ms | 815.748 ms | 1102.454 ms |
| Completion checkpoint application-pool wait | 20 | 0.001 ms | 0.002 ms | 0.002 ms | 0.002 ms | 0.002 ms |
| Event-loop lag | 128 | 0.028 ms | 1.034 ms | 24.701 ms | 367.068 ms | 555.006 ms |
...
**555.006 ms**. It remained below the qualification-only five-second component
...
|---|---:|---|---:|---:|---:|---:|
| Admission | 100,000/hour | 200 every 5 seconds | 144,000/hour | 44% | 0.337261 s | 2,134,844.329/hour |
| Run barrier | 10,000/hour | 20 every 5 seconds | 14,400/hour | 44% | 0.203542 s | 353,734.622/hour |
...
The run-barrier release batch and candidate budget were both 20. The first
pass examined a blocked 20-run prefix, released nothing, and persisted its
cursor at the end of that budget. The measured second pass resumed immediately
after the prefix, examined 20 later candidates, and released all 20. This is
the qualification's direct evidence that the bounded cursor rotates fairly
past an ineligible prefix rather than rescanning it.
...
returned only `planner-00000-eligible` in **0.325 ms** under normal planner
...
memberships. It selected 20 runs and their 40 memberships in **0.144 ms**, using
...
The qualification ran on PostgreSQL 17.9 (Homebrew) through the local Unix
socket database `dr_platform_test`, with a clean tree at both recorded
Git-status checks.
...
The run completed at `2026-08-09T23:36:21.918730+00:00` on branch
