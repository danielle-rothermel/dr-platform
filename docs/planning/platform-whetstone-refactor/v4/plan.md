# Platform Hard Cut — Joint Refactor Spec (v4)
...
V4 was re-audited against these exact working trees:
...
[`traceability.md`](traceability.md) is non-normative. It records v0-v3
...
| Whetstone | Prediction, Generation Run, Score Attempt, and Experiment identity; domain eligibility and outcomes; concrete opaque recipe payloads; generation/scoring selection; accepted Manifest relationships; acceptance evaluation; provider/model/cost facts linked to persisted outcomes. |
...
5. Whetstone scoring and Experiment acceptance move together: immutable
   selections, successive scoring Operations, representable missing cells,
   highest-success selection, accepted Manifest relationships, and checked
   source/platform cuts precede deletion of custom batching/replay.
...
- generation/scoring/domain/acceptance separation, later-Attempt behavior,
  explicit missing cells, highest-success selection, and stale-cut rejection;
...
V4 remains mutable as `draft` until the owner explicitly starts review. The
...
effort index to `in-review` without changing contract content; v4 then freezes,
...
The existing [Codex code/dependency prompt](reviews/codex-prompt.md) and
[Claude architecture/domain prompt](reviews/fable-prompt.md) predate this
normalization and are retained unchanged. They are not issued and must be
refreshed by a later worker to name `plan-manifest.json`, the full ordered
normative set, and the separate traceability closure pass before review can
start. Findings, unified feedback, and a review baseline do not exist.
