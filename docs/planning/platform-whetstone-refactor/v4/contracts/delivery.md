...
5. **Whetstone scoring cut.** Add scoring Item/Operation identity and target,
   ...
   command, add accepted Manifest relationships, representable generation/
   scoring/candidate acceptance members, highest-success selection, and the
   domain-source/platform-cut/current-pointer transaction, and prove harness-failed
...
- acceptance evaluation against exact generation and multiple scoring Manifest
  digests; explicit missing Generation and missing Score cells; accepted
  Manifest relationship finalization; next-Attempt reactivation,
...
- cancel during Claim and cancel followed by late enqueue, proving Claim
  ...
  compensation, and an append-only compensation record;
...
2. **Generation identity:** overlapping Operations for identical Predictions
   ...
   target registration fails closed.
3. **Generation/scoring lifecycle:** one experiment creates linked generation
   ...
   explicit cells. Shared late success plus cancel/retry selects only the
   highest successful ordinal for required scoring and preserves earlier
   successes as superseded provenance. A later relevant domain outcome clears
...
4. **Operator readiness:** list/show/items/attempts/workflow/queue/throttle and
   ...
   `NOT_ENQUEUED` versus delivered cancellation, and a confirmed later-Attempt
...
V4 intentionally does not add export-aware DBOS retention, raw replay/resume/
