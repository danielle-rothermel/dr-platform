...
- **Standard tables (kernel-owned):** `<prefix>_operations`, `<prefix>_items`,
  ...
  The allowlisted DBOS telemetry projection is a full rebuild for v4: a
...
- **Client augmentation:** apps register domain projections through a frozen
  ...
  detail is not an Analysis member. A v4 client
...
- **Sinks:** (1) MotherDuck sync of the DuckDB file/database — the Analysis
  ...
  complete. The selection uses a versioned stable hash and threshold; v4
...
table in v4; workflow/step details are retrieved on demand through the typed
inspector.
