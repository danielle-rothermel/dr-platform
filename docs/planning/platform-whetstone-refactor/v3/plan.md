# Platform Hard Cut — Joint Refactor Spec (v3)
...
V3 was re-audited against these exact working trees:

- `dr-platform`: branch `07-08-refactor`,
  `7b9b340fd8f2717e44de36804396077b7beeb661`. The tree contains only this
  issued planning effort's index, canonical glossary/ADR updates, immutable v1
  and v2 review packets, and the v3 prompt/draft; no application code, tests,
  dependency files, or runtime configuration changed.
...
No application-code revision has moved since the v2 convergence review, so
...
- `<prefix>_item_attempts`: append-only lineage keyed by
  ...
  execution outcome, lease fields
...
| Current Operation column | V3 column | Action and invariant |
| --- | --- | --- |
...
| Current Item column | V3 column/location | Action and invariant |
| --- | --- | --- |
...
| Current surface | V3 surface | Action |
| --- | --- | --- |
...
`OperationManifest` is a frozen Pydantic model with `format_version = 2`,
`operation_key`, `workflow_role`, `group_key`,
`operation_execution_recipe_digest`,
...
The target also supplies frozen, scoped execution recipes. Each concrete
`ItemExecutionRecipe` contains that Item's complete domain input snapshot;
managed workflow name, implementation/version, and topology; argument-recipe
version; and every relevant profile, parser, dataset, graph,
provider-configuration, and application version that can affect execution.
Its full lowercase SHA-256 `execution_recipe_digest`, computed with
dr-serialize canonical JSON, is persisted on the Attempt and participates in
content-scoped execution/workflow identity. The Operation persists an
`operation_execution_recipe_digest` over the target recipe plus the complete
ordered list of concrete Item recipe digests; exact resubmission requires both
that digest and the Manifest digest. Callable objects themselves are not
serialized and are never treated as identity; their stable declared names and
versions are.
...
lock and checks for any unresolved DBOS-cancellation intent on an Attempt with
...
Attempt as DBOS cancel requested, skipped-shared, observed terminal, or
...
duplicate spend; cost reconciliation retains both calls when observable and
inspection distinguishes `DBOS_CANCELLED` from `UPSTREAM_ABORT_PROVED` (the
latter is not supported by the pre-experiment adapters). See
...
- **Standard tables (kernel-owned):** `<prefix>_operations`, `<prefix>_items`,
  ...
  The allowlisted DBOS telemetry projection is a full rebuild for v3: a
...
- **Client augmentation:** apps register domain projections through a frozen
  Pydantic projection contract carrying build callables — e.g. Whetstone's
  predictions/generation-runs/score-attempts projections. A v3 client
...
- **Sinks:** (1) MotherDuck sync of the DuckDB file/database — the Analysis
  ...
  complete. The selection uses a versioned stable hash and threshold; v3
...
Whetstone's fresh baseline adds two append-only tables and an Experiment
pointer:
...
- `experiments` adds positive `acceptance_source_version` (starting at 1),
  ...
  current only when its evaluation's source version equals the Experiment's.
...
- `experiment_acceptance_members` is keyed by
  `(acceptance_id, prediction_id, generation_run_id, scoring_profile_id,
  scoring_profile_version, parser_profile_id, parser_version, dataset_name,
  dataset_split)`. Each expected matrix cell records its selected nullable
  `score_attempt_id`, domain disposition, exact Generation and Scoring
  Operation/Item/Attempt references, and the contributing Manifest digests.
  Foreign keys target the append-only Whetstone domain rows and platform
  references; members are immutable and cannot outlive their evaluation.
...
one repeatable source cut and version, derives the full expected/observed
matrix across all contributing Scoring Operations, inserts or reloads the
exact immutable evaluation and members, then advances the Experiment pointer
with `WHERE experiment_name=:name AND acceptance_source_version=:read_version`.
The pointer update and evaluation insertion commit atomically. A source-version
CAS loss leaves the new evaluation historical, returns `SOURCE_ADVANCED`, and
requires reevaluation; it never promotes a known-stale result. Exact replay
returns the same evaluation and does not duplicate members. Readers use only
the Experiment pointer for current acceptance and may request historical
evaluations explicitly; they never infer currentness from maximum timestamp.
...
- `optimization/copro.py` is repointed minimally to the typed lifecycle and
  ...
  local-DuckDB Analysis adapter that refuses to drift to a newer bundle during
  the iteration. It never queries operational Postgres for analysis and export
...
table in v3; workflow/step details are retrieved on demand through the typed
inspector.
...
5. **Whetstone scoring cut.** Add scoring Item/Operation identity and target,
   ...
   command, add append-only acceptance evaluations/members plus the
   source-version/current-pointer transaction, and prove harness-failed
   rescoring plus strict and explicit partial Experiment acceptance. Prove
   stale-pointer invalidation and candidate parity with current rescore fixtures
...
- acceptance evaluation against exact generation and multiple scoring Manifest
  digests; concurrent outcome insertion versus pointer promotion; later
  Generation Run, Score Attempt, Manifest relation, and required-profile
  invalidation; exact evaluation replay; historical lookup; biased partial
  override provenance; and `SOURCE_ADVANCED` without stale pointer advance;
...
- reference-aware cancellation with exclusive, shared, newly racing,
  ...
  overlap it and that both observable costs remain accounted;
...
3. **Generation/scoring lifecycle:** one experiment creates linked generation
   ...
   immutable member matrix. A later relevant domain outcome clears the current
   pointer; reevaluation appends and atomically promotes a new evaluation while
   preserving the former result as historical.
4. **Operator readiness:** list/show/items/attempts/workflow/queue/throttle and
   ...
   cancellation, and a confirmed later-Attempt authorization. The gate injects
   cancellation during a synchronous paid call and proves that any overlap is
   labeled and accounted rather than claiming physical upstream stop.
5. **Queue/pacing:** exact DBOS 2.26.0 status/API/schema contracts and priority
   config are verified; deterministic kernel ranks and bounded pre-enqueue
...
V3 intentionally does not add export-aware DBOS retention, raw replay/resume/
...
| Priority | Unified item | V3 retained resolution |
| --- | --- | --- |
...
| Priority | Unified item | V3 disposition |
| --- | --- | --- |
...
| P0-2 | Immutable registration Manifest | Retained caller-prepared Manifest mechanics; added concrete Item execution-recipe identity, ordered Operation recipe digest, and exact canonical domain-row equality. |
...
| P0-4 | Complete cancellation topology | Retained top-level-only reference locking and non-recursive DBOS cancellation; added foreign provenance and race outcomes. Owner explicitly accepts overlapping paid calls after logical cancellation. |
| P0-5 | Experiment acceptance | Retained strict/stratified policy; added append-only evaluations/members, exact source cuts, invalidation version, and one atomic current pointer. |
...
| Priority | V2 synthesis finding | V3 disposition |
| --- | --- | --- |
| P0-1 | Complete execution recipe and exact domain equality | Owner chose separate versioned concrete recipe digests. Attempt/workflow identity includes the Item digest; Operation and Manifest equality include the ordered aggregate; `ALREADY_PRESENT` requires exact canonical domain equality. |
...
| P0-3 | Paid-call cancellation/quiescence | Owner rejected quiescence and accepts duplicate spend. Logical DBOS cancellation permits confirmed replacement; overlap is labeled, tested, and included in cost reconciliation. |
...
| P0-5 | Durable Experiment acceptance | Append-only evaluation/member schema, exact Manifest/domain/platform cuts, source-version invalidation, immutable history, and atomic current-pointer CAS are specified. |
...
Reviewer disagreements remain visible in the owner decisions below. Codex's
identity, bundle, cancellation, acceptance, and DBOS-ordering concerns caused
v3; Fable's closure conclusions remain recorded where the owner preserved the
v2 mechanism and added a narrower completion. The status-precedence split is
resolved in Fable's favor by adding the missing happy-path clause. The overall
gate remains another whole-system convergence review.
...
3. Treat DBOS cancellation of synchronous paid steps as logical and allow a
   ...
   recommendation in favor of a smaller lifecycle contract that labels and
   accounts for overlap but does not prevent it.
...
V3 is frozen as `in-review` for another independent whole-system convergence
review over dr-platform, Whetstone, Unitbench, DBOS, export, and the runtime
constellation. Findings may not be patched into v3; decision-changing findings
land only in a successor draft. No focused audit or implementation begins
before this convergence review completes.

The next-round [Codex code/dependency prompt](reviews/codex-prompt.md) and
[Claude architecture/domain prompt](reviews/fable-prompt.md) are issued against
this frozen content. Any content or baseline drift requires a successor draft
or refreshed prompt as appropriate. Findings and unified feedback do not exist
yet.
