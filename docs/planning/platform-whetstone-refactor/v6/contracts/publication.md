...
- **Standard tables (kernel-owned):** `<prefix>_operations`, `<prefix>_items`,
  `<prefix>_item_attempts`, `<prefix>_enqueue_claims`,
  `<prefix>_next_attempt_requests`,
  ...
  The allowlisted DBOS telemetry projection is a full rebuild for v6: a
...
- **Client augmentation:** apps register domain projections through a frozen
  ...
  detail is not an Analysis member. A v6 client
...
- **Sinks:** (1) MotherDuck sync of the DuckDB file/database — the Analysis
  ...
  complete. The selection uses a versioned stable hash and threshold; v6
...
table in v6; workflow/step details are retrieved on demand through the typed
inspector.
