...
- A Scoring Operation records its source Generation Operation key in its
  ...
  generation, selects eligible `SUCCESS` runs and only `PARTIAL` runs whose
  persisted `terminal_submission_text` satisfies the exact SQL predicate
  `IS NOT NULL AND ~ '[^[:space:]]'`, and explicitly submits the Scoring
  Operation. Empty and POSIX-whitespace-only `PARTIAL` rows are excluded
  before selection digest, Item identity, or Manifest construction; they have
  no scoring retry or outcome in the replacement path. This intentionally
  narrows parity from the legacy status-only selector to the populated-only
  candidates that the legacy scoring path could actually score. `PARTIAL`
  eligibility does not make that run a strict accepted Generation outcome;
  strict acceptance still requires `SUCCESS` unless a separately persisted
  explicit acceptance policy says otherwise. The kernel does not model a DAG
  or auto-start dependent Operations.
...
- The experiment-facing command reports both Operation statuses and one
  ...
  logical Prediction/profile/parser/dataset cell, the cell first pins the
  evaluation's selected accepted Generation Run for that Prediction. Only
  Score Attempts whose `generation_run_id` equals that pinned run are eligible
  to win. Acceptance then chooses the newest relationship, by its persisted
  monotonic accepted-Scoring-relationship ordinal, that has at least one
  successful run-matched candidate; within that relationship's single scoring
  Item lineage it chooses the successful Score Attempt with the highest
  platform Attempt ordinal. Newer relationships with no run-matched successful
  candidate do not erase an older run-matched success. Candidates for every
  other Generation Run remain immutable `SUPERSEDED_GENERATION` provenance and
  can never satisfy or win the cell. A persisted partial/override policy that
  explicitly accepts a `PARTIAL` Generation Run pins that run by the same rule;
  it does not make other runs interchangeable.

  Strict pointer promotion also requires execution closure. Every Generation
  or Scoring Operation named by an accepted Manifest relationship must be in a
  terminal platform aggregate state before reduction, so a later workflow
  completion cannot change candidate eligibility. Each selected Generation
  Run and Score Attempt must reference its exact platform Attempt in terminal
  `SUCCEEDED` state with normalized DBOS `SUCCESS`. A terminal Operation may be
  `PARTIAL`, `FAILED`, or `CANCELLED` only when it contributes no selected
  candidate and its immutable terminal candidates are retained as rejected or
  superseded provenance. A nonterminal Operation, nonterminal selected Attempt,
  recovery exhaustion, missing workflow, or incompatible terminal Attempt
  returns typed `EXECUTION_NOT_TERMINAL` or `EXECUTION_INCOMPATIBLE`, leaves the
  evaluation historical/partial, and cannot promote `current_acceptance_id`.
  Persisted override policy may relax domain completeness only; it cannot
  waive terminal platform/DBOS success for a candidate it selects.
...
- `experiment_acceptance_scoring_candidates` is keyed by the acceptance ID,
  ...
  observed domain status, pinned accepted `generation_run_id`, and one of
  `SELECTED`, `SUPERSEDED_GENERATION`, `SUPERSEDED_RELATIONSHIP`,
  `SUPERSEDED_ATTEMPT`, or `REJECTED`. It records all
...
accepted Generation and Scoring Operation. Before candidate reduction it
requires every relationship Operation to be terminal and exact selected
Attempts to be terminal `SUCCEEDED` with DBOS `SUCCESS`. It derives the full expected
...
lineage, pins each cell to its selected accepted Generation Run, marks all
other-run candidates `SUPERSEDED_GENERATION`, and chooses score winners by
newest run-matched successful accepted-Scoring relationship then highest
successful Attempt ordinal inside that relationship's Item lineage. It persists
...
- Before deleting the old rescore path, fixtures pin its current selection:
  ...
  The replacement Manifest must match the deliberately narrowed retained set:
  every status-eligible `SUCCESS` plus only `PARTIAL` rows satisfying
  `terminal_submission_text IS NOT NULL AND terminal_submission_text ~
  '[^[:space:]]'`. Empty and space-, tab-, or
  newline-only `PARTIAL` fixtures must be excluded before selection identity;
  a populated fixture must be included. Candidate identity, retry behavior,
  scoring outcome, and deletion checks use this same predicate before the old
  SQL and batching flow are removed. A paired acceptance fixture proves that
