# Platform Hard Cut — Joint Refactor Spec (v6)
...
V6 carries forward v5's reviewed re-audit against these exact working trees;
...
8. **Attempt state is not Claim history.** An Attempt may point to at most one
   current enqueue Claim, but every Claim that reached the enqueue-call
   boundary has its own immutable durable identity. Replacement,
   invalidation, and terminalization never erase the exact Claim key used by
   compensation and replay.
...
[`traceability.md`](traceability.md) is non-normative. It records v0-v5
...
9. Accepted Scoring relationships receive immutable monotonically increasing
   ordinals per Experiment. Each logical scoring cell is first pinned to the
   evaluation's selected accepted Generation Run. Among only run-matched
   candidates, acceptance selects from the newest relationship with a
   successful candidate, then the highest successful platform Attempt ordinal
   within that relationship and Item lineage. Other-run candidates persist as
   `SUPERSEDED_GENERATION` provenance and cannot satisfy or win the cell. The
...
10. Scoring selection adopts populated-only `PARTIAL` eligibility. The one
    canonical persisted-output predicate is `terminal_submission_text IS NOT
    NULL AND terminal_submission_text ~ '[^[:space:]]'`. Empty and POSIX-whitespace-only
    `PARTIAL` rows are excluded before Manifest construction and never receive
    scoring Item identity, retry behavior, or a scoring outcome. Deletion
    parity is intentionally narrowed to the retained populated-only candidate
    set rather than the legacy status-only selector. This eligibility remains
    distinct from strict Generation acceptance: `PARTIAL` does not satisfy
    strict acceptance unless a separate explicit persisted policy authorizes
    it.
...
4. The kernel claims eligible current Attempts in deterministic service-class
   ...
   content-scoped workflow ID, and persists the outcome with a Claim CAS. Each
   Claim/enqueue-call boundary is first recorded append-only; later Claim
   replacement or Attempt terminalization cannot erase it. Existing executions
   are linked without rewriting immutable attributes.
...
7. Reference-aware cancellation records sticky local intent first, physically
   ...
   records the append-only compensation row against its durable Claim. Both
   claimant and replay compensation recheck reference exclusivity under the
   standard lock order; shared work resolves `SKIPPED_SHARED` without a DBOS
   call, and bounded missing-workflow observation resolves
   `NO_WORKFLOW_FOUND` rather than leaving an indefinite link hazard.
...
- generation/scoring/domain/acceptance separation, later-Attempt behavior,
  ...
  deterministic score selection across ordered relationships pinned to the
  selected accepted Generation Run, populated-only `PARTIAL` scoring
  eligibility versus strict acceptance, terminal platform execution for
  selected candidates and contributing Operations, highest-success selection
  within each chosen lineage, and stale-cut rejection;
- typed operator readiness, non-recursive cancellation, durable append-only
  Claim identities, Claim invalidation, reference-safe late-enqueue
  compensation with bounded hazard resolution, and honest accepted paid-call
  overlap;
...
V6 remains mutable as `draft` until review starts. The
...
effort index to `in-review` without changing contract content; v6 then freezes,
...
The two v6 review prompts are prepared. No baseline, finding, synthesis, or
placeholder exists. V5 review outputs remain historical evidence, not v6
results.
