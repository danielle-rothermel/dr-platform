...
2. **Platform vocabulary and baseline.** Implement fixed naming, Pydantic
   ...
   ledgers, the complete schema crosswalk, internally owned shared writer lock,
...
3. **Platform lifecycle.** Implement caller-prepared Manifest validation,
   ...
   compensation, and non-recursive reference-aware cancellation. A fresh
...
5. **Whetstone scoring cut.** Add scoring Item/Operation identity and target,
   ...
   and score selection with full supersession provenance, and the
   domain-source/platform-cut/current-pointer transaction. Prove harness-failed
   rescoring, populated `PARTIAL` Generation Run scoring eligibility, and its
   ...
   candidate parity with current rescore fixtures
...
- scoring-cell selection across accepted Scoring relationships for
  ...
  recipes, and concurrent relationship/evaluation races, proving newest
  relationship with a success then highest successful Attempt, complete
  candidate/supersession persistence, and identity sensitivity to relationship
  order and selected inputs;
- current rescore candidate parity for `SUCCESS` and populated `PARTIAL`
  Generation Runs, proving `PARTIAL` remains scoring-eligible while failing
  strict Generation acceptance absent a separately persisted explicit policy;
...
- cancel during Claim and cancel followed by late enqueue, proving Claim
  ...
  reconciliation discovers the workflow and creates/replays that compensation;
...
3. **Generation/scoring lifecycle:** one experiment creates linked generation
   ...
   newest accepted Scoring relationship containing a success wins, then its
   highest successful Attempt wins; all candidates and supersession facts are
   persisted and bound into acceptance identity. Populated `PARTIAL`
   Generation Runs remain eligible for scoring-selection parity but do not
...
4. **Operator readiness:** list/show/items/attempts/workflow/queue/throttle and
   ...
   claimant-death compensation discovery, `NOT_ENQUEUED` versus delivered
...
V5 intentionally does not add export-aware DBOS retention, raw replay/resume/
