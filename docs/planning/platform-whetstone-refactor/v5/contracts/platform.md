...
- `<prefix>_enqueue_compensations`: append-only idempotency ledger keyed by
  `(item_id, attempt, claim_id)`. It records a claimant's DBOS enqueue side
...
- A shared Postgres sequence plus trigger assigns a unique `change_seq` on
  ...
  Item Attempts, next-Attempt requests, enqueue compensations, and throttle
  state). The baseline
...
| Current Operation column | V5 column | Action and invariant |
| --- | --- | --- |
...
| Current Item column | V5 column/location | Action and invariant |
| --- | --- | --- |
...
conflict. Indexes cover `workflow_id`, unresolved disposition, and
`change_seq`.
...
| Current surface | V5 surface | Action |
| --- | --- | --- |
...
every outstanding enqueue Claim for affected Attempts. When no DBOS row
exists, finalization records `NOT_ENQUEUED`; it never describes DBOS's silent
no-op as a delivered cancellation. If an in-flight claimant enqueues after
Claim invalidation, its outcome CAS necessarily loses. It reloads the
cancellation result, calls idempotent
`cancel_workflow(workflow_id, cancel_children=False)` now that the DBOS row
exists, and inserts or replays the append-only enqueue-compensation row before
returning. Unresolved compensation keeps health degraded and prevents the
cancellation command from reporting fully resolved. This race is distinct
from an already-running synchronous provider call continuing after logical
cancellation.
...
deterministic workflow ID was persisted before Claim invalidation. If DBOS now
contains that workflow but no resolved compensation row exists, reconciliation
inserts or exact-reloads the same `(item_id, attempt, claim_id)` compensation
ledger entry and issues the same idempotent non-recursive cancellation. This
repairs claimant death after enqueue but before the losing outcome CAS. An
unresolved or unequal compensation keeps health degraded and the Operation's
cancellation incomplete; terminal Attempt fields remain immutable.
...
- The root export inventory is explicit: schema/config (`PlatformSchema`,
  ...
  cancellation-compensation record (`EnqueueCompensationRecord`);
